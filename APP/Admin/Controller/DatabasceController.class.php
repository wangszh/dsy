...
class DatabasceController extends AdminController{
    ...
    public function optimize($tables = null){
        if($tables) {
            ...
            if(is_array($tables)){
                ...
                if($list){
                    $this->addLog('tables='.$tables,1);// 记录操作日志
                    $this->success("数据表优化完成！");
                } else {
                    $this->addLog('tables='.$tables,0);// 记录操作日志
        ...
        if(is_numeric($time) && is_null($part) && is_null($start)){ //初始化
            ...
            if(false === $start){
                ...
                if(isset($list[++$part])){
                    ...
                    $this->addLog('还原数据库',1);// 记录操作日志
                    $this->success('ok');
                }
            } else {
                $data = array('part' => $part, 'start' => $start[0], 'size' => $start[1]);
                if($start[1]){
                    $rate = floor(100 * ($start[0] / $start[1]));
                    $this->success("正在还原...#{$part} ({$rate}%)", '', $data);
                } else {
                    $data['gz'] = 1;
                    $this->success("正在还原...#{$part} ($start[0])", '', $data);
                }
            }
